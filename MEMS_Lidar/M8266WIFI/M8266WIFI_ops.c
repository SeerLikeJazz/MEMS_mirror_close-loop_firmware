...
/////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   // (Chinese:第5步：配置模组)
	...
	{      // (Chinese: 5.2 如果你希望配置或关闭模组上自带的WEB服务器，你可以将这里改成 #if 1，并调整下面的API函数里的相关参数值)
		   ...
		   if(M8266WIFI_SPI_Set_WebServer(0, 3128, 1, &status)==0)  // 0 -> to shutdown the web_server (Chinese: 第一个参数0表示关闭WEB服务器)
		   {
				  return 0;
		   }
       //u8 M8266WIFI_SPI_Set_WebServer(u8 open_not_shutdown, u16 server_port, u8 saved, u16* status)
...
M8266WIFI_SPI_Query_AP_Param(AP_PARAM_TYPE_SSID, ssid, 13+1, &status);
char ssid_iRe[14]={'i', 'R', 'e', '-'};
for(uint8_t i=4;i<10;i++){
	ssid_iRe[i]  = ssid[i];
}

#if 1 // 5.4 If you expect to change the ap info overrding the default loaded from flash on bootup, enable it by "#if 1"
{     // (Chinese: 5.4 如果你希望改变模组作为AP热点时AP热点名称和密码，不使用模组启动时缺省参数，你可以这里改成 #if 1，并调整下面的API函数里的相关参数值)
// u8 M8266WIFI_SPI_Config_AP(u8 ssid[13+1], u8 password[13+1], u8 enc, u8 channel, u8 saved, u16* status);
	if(M8266WIFI_SPI_Config_AP(ssid_iRe, "1234567890", 0, 1, 0, &status)==0)  // set to 4=WPA_WPA2_PSK, not saved // 0=OPEN, 1=WEP, 2=WPA_PSK, 3=WPA2_PSK, 4=WPA_WPA2_PSK
    return 0;
}
#endif
...
//                          if AP mode or STA+AP mode, then optimized to select the AP channel
//(Chinese: 5.5 查询当前的op_mode，如果处于STA模式或者STA+AP模式，那么可根据需要执行配网去链接上第三方热点/路由器，并等待获取ip地址)
//                                 如果处于AP模式或者STA+AP模式， 那么可根据需要，配合周围环境进行AP频道的适配优化选择
//u8 M8266WIFI_SPI_Get_Opmode(u8* op_mode, u16* status);
	if(M8266WIFI_SPI_Get_Opmode(&sta_ap_mode, &status)==0)
    return 0;

#if 1  // If you expect to use an optimal AP channel, enable here by "#if 1".
		  // (Chinese: 如果你希望对AP频道根据环境进行AP频道的适配优化选择，你可以这里改成 #if 1。否则，可以改成#if 0，不执行这项操作)	
	   ...
	   ||(sta_ap_mode == 3))  // if STA+AP mode(Chinese: 如果是STA+AP模式)
	{
       ...
       if(M8266WIFI_SPI_STA_Connect_Ap("niantong", "niantong", 0, 20, &status) == 0) // not saved, timeout=20s
				 return 0;
#endif
